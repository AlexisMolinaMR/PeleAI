...
def parseArg():
    ...
    parser.add_argument("-m", "--metric", required=True, type=str, help="metric to plot: total energy, hbond, rmsd or docking distance." )
...
def scatter(path, metric, save_path, mode, filter):
    ...
    fields = ['hbond_H_val_690', 'RMSD_ligand', 'docking_com_dist', 'Binding Energy', 'currentEnergy', 'trajectory', 'epoch', 'numberOfAcceptedPeleSteps'] # to be given as input (argparse)
    ...
    for filename in glob.glob(os.path.join(path, '*.csv')):
        ...
        if mode == "normal" and filter is None:
            if metric == "hbond":
                ax = sns.scatterplot(pele_out_summary.hbond_H_val_690, pele_out_summary.Binding_Energy)
                plt.xlabel("H-bond")
            elif metric == "rmsd":
                ax = sns.scatterplot(pele_out_summary.RMSD_ligand,pele_out_summary.Binding_Energy)
                plt.xlabel("RMSD")
            elif metric == "docking":
                ax = sns.scatterplot(pele_out_summary.docking_com_dist, pele_out_summary.Binding_Energy)
                plt.xlabel("Docking distance")
            elif metric == "energy":
                ax = sns.scatterplot(pele_out_summary.Binding_Energy, pele_out_summary.currentEnergy)
        ...
        if mode == "normal" and filter is not None:
            ...
            if metric == "hbond":
                ax = sns.scatterplot(filtered_dataframe.hbond_H_val_690, filtered_dataframe.Binding_Energy)
                plt.xlabel("H-bond")
            elif metric == "rmsd":
                ax = sns.scatterplot(filtered_dataframe.RMSD_ligand, filtered_dataframe.Binding_Energy)
                plt.xlabel("RMSD")
            elif metric == "docking":
                ax = sns.scatterplot(filtered_dataframe.docking_com_dist, filtered_dataframe.Binding_Energy)
                plt.xlabel("Docking distance")
            elif metric == "energy":
                ax = sns.scatterplot(pele_out_summary.Binding_Energy, pele_out_summary.currentEnergy)
                plt.xlabel("Total Energy")
            ...
            if metric == "hbond":
                ...
                fig = go.Figure(data=go.Scatter(x=pele_out_summary.Binding_Energy,
                                y=pele_out_summary.currentEnergy,
                                mode = 'markers',
                                text = pele_out_summary.trajectory +'<br>'+ 'Epoch: '+ pele_out_summary.epoch +'<br>'+ 'Step: ' + pele_out_summary.numberOfAcceptedPeleSteps,
                                marker=dict(
                                    size=10,
            ...
            if save_path is None:
                ...
                plo.offline.plot(fig, filename = save_path + '/' + pele_out_summary.trajectory[0].split('/')[0] + '_' + metric + "_interactive_pele_plot.html", auto_open=False)
                print("Writing image " + pele_out_summary.trajectory[0].split('/')[0] + ".html" + " to: " + save_path)
